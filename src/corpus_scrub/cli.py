...
from corpus_scrub.report import write_report, build_report
...
def main(argv=None) -> int:
    ...
    scan.add_argument("--ner-threshold", type=float, default=0.85,
                      help="min NER score to keep (default 0.85)")
    ...
    if args.command == "scan":
        if args.lang != "en":
            ...
            print(f"ERROR: language '{args.lang}' no soportado en MVP (solo 'en'). "
                  f"Ver spec.md KI-1 / Fase 2.", file=sys.stderr)
            return 2
        findings, redacted_docs = _scan(
            args.input, args.lang, args.policy, args.ner_threshold
        )
        if args.report:
        ...
        print(f"Documentos: {len(redacted_docs)} | Hallazgos: {summary['total_findings']} "
              f"| Por tipo: {summary['counts_by_type']}", file=sys.stderr)
        return 0
